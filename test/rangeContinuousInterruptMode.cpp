...
void setup() {
  ...
  /*Set threshold value*/
  VL6180X.setRangeThresholdValue(/*thresholdL 0-255mm */100,/*thresholdH 0-255mm*/255);
   ...
   * |-------------------------------------------------------------------------------------------------------------------------------------------|
   */
  attachInterrupt(/*Interrupt No*/1,interrupt,FALLING);//Enable the external interrupt 1, connect INT1/2 to the digital pin of the main control: 
  ...
  if(flag == 1){
    ...
    // if(VL6180X.rangeGetInterruptStatus() == VL6180X_OUT_OF_WINDOW){
     ...
     // uint8_t status = VL6180X.getRangeResult();
      ...
      flag = 0;
      String str1 = "Range: "+String(count) + " mm"; 
      Serial.println(str1);
    /*  switch(status){
        case VL6180X_NO_ERR:
          Serial.println(str1);
